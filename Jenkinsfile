...
pipeline {
    ...
    stages {
        ...
        tage('Deploy to AWS Ubuntu Server') {
            steps {
                ...
                sh '''
                scp -o StrictHostKeyChecking=no -r ./ ubuntu@18.209.106.29:/var/www/myapp
                ssh ubuntu@18.209.106.29 'pm2 restart myapp || pm2 start /var/www/myapp/index.js --name myapp'
                '''
            }
        }
    }
